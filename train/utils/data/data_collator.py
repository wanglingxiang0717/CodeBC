    ...
    def decoder_call(self, batch, return_tensors):
        ...
        for instance in batch:
            ...
            if not self.inference:
                ...
                tokenized_instruction = self.tokenize(instruction, limit_len, add_bos_token=True, add_eos_token=False)
                tokenized_label = self.tokenize(label, limit_len, add_bos_token=False, add_eos_token=True)
...
class DataCollatorForGLUE:
    ...
    def __call__(self, batch, return_tensors=None):
        if return_tensors is None:
            return_tensors = self.return_tensors
        return self.collate_batch(batch, return_tensors)

    def collate_batch(self, batch, return_tensors="pt"):
        if 'sentence' in batch[0]:
            texts = [x['sentence'] for x in batch]
            text_pair = None
        elif 'sentence1' in batch[0] and 'sentence2' in batch[0]:
            texts = [x['sentence1'] for x in batch]
            text_pair = [x['sentence2'] for x in batch]
        else:
            raise ValueError("Batch must contain 'sentence' or ('sentence1' and 'sentence2')")
