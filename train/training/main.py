...
#     return checkpoint.checkpoint_orig(function, *args, **kwargs)

# checkpoint.checkpoint_orig = checkpoint.checkpoint
...
from params import Method2Class
...
def parse_args():
    ...
    parser.add_argument('--forze_embedding',
                action='store_true',
                help='Sequential or Random')
...
def main():
    ...
    if args.CL_method == "lora":
        ...
        model = get_peft_model(model, peft_config)
        # model = LoraModel(model, peft_config, adapter_name="default")
        for name, param in model.named_parameters():
            if name.find("lora") != -1:
                param.requires_grad = True        
